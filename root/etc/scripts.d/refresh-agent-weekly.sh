...
curl -sL "https://download.db-ip.com/free/dbip-country-lite-$(date +%Y-%m).mmdb.gz"  \
  -o /tmp/refresh-agent/geoip.mmdb.gz 2>&1 | tee -a $LOG_FILE

# Check if GeoIP db for this month was already released, if not download version of previous month
GEOIP_DB_SIZE=$(du -sb "/tmp/refresh-agent/geoip.mmdb.gz" | awk '{ print $1 }')
...
gunzip -f /tmp/refresh-agent/geoip.mmdb.gz
mkdir -p /usr/share/GeoIP
mv -f /tmp/refresh-agent/geoip.mmdb /usr/share/GeoIP/geoip.mmdb
